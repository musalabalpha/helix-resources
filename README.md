...
- [`punto-de-guardado/`](skills/punto-de-guardado/) — guardar el trabajo de una sesión con un agente sin cerrar el día: control de versiones sin saltos silenciosos (lo ajeno en commit aparte), tareas tocadas al día y un puntero `RETOMA` en las notas para que el siguiente arranque no herede un estado que miente.
- [`cierre-del-dia/`](skills/cierre-del-dia/) — ritual de fin de jornada: balance, guardado, reconciliación de lo entregado contra el tablero, lista de mañana y procesos que quedaron corriendo. Reúsa `balance-de-sesion` y `punto-de-guardado` en vez de copiar sus reglas.
